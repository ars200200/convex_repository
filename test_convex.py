...
class TestFigure_intersects:
    # Измерение пощади пересечения двух треугольников
    # один из треугольников не дан
    def test_intersects(self):
        a = R2Point(0.0, 0.0)
        b = R2Point(1.0, 0.0)
        c = R2Point(0.0, 1.0)
        assert Figure.intersects([a, b, c]) is None

    # Площадь перечесения двух одинковых треугольников
    # равна площади одного из них
    def test_intesects2(self):
        a1 = R2Point(0.0, 0.0)
        b1 = R2Point(1.0, 0.0)
        c1 = R2Point(0.0, 1.0)
        a2 = R2Point(0.0, 0.0)
        b2 = R2Point(0.0, 1.0)
        c2 = R2Point(1.0, 0.0)
        assert Figure.intersects([a1, b1, c1], [a2, b2, c2]) == 0.5

    def test_intesects3(self):
        a1 = R2Point(0.0, 0.0)
        b1 = R2Point(1.0, 0.0)
        c1 = R2Point(0.0, 1.0)
        a2 = R2Point(0.0, 0.0)
        b2 = R2Point(0.0, 1.0)
        c2 = R2Point(1.0, 1.0)
        assert Figure.intersects([a1, b1, c1], [a2, b2, c2]) == 0.25
...
class TestPoint:
    ...
    # Инициализация (выполняется для каждого из тестов класса)
    def setup_method(self):
        self.f = Point(R2Point(0.0, 0.0), 0)
...
class TestSegment:
    ...
    # Инициализация (выполняется для каждого из тестов класса)
    def setup_method(self):
        self.f = Segment(R2Point(0.0, 0.0), R2Point(1.0, 0.0), 0)
